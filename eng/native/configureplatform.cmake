...
if(CLR_CMAKE_HOST_OS STREQUAL linux)
    ...
    if(DEFINED CLR_CMAKE_LINUX_ID)
        if(CLR_CMAKE_LINUX_ID STREQUAL tizen)
            set(CLR_CMAKE_HOST_TIZEN_LINUX 1)
    ...
    # KOS: not an elseif of the Linux ID check above. cmake_host_system_information defines
    # CLR_CMAKE_LINUX_ID (empty) even when the sysroot has no /etc/os-release, as the KOS sysroot doesn't.
    if(EXISTS ${CROSS_ROOTFS}/sysroot-${CMAKE_SYSTEM_PROCESSOR}-kos)
        set(CLR_CMAKE_TARGET_OS KOS)
        set(CLR_CMAKE_TARGET_UNIX 1)
        set(CLR_CMAKE_TARGET_LINUX 1)
        set(CLR_CMAKE_TARGET_KOS 1)
    endif()
...
if (NOT (CLR_CMAKE_TARGET_OS STREQUAL CLR_CMAKE_HOST_OS) AND NOT CLR_CMAKE_TARGET_WASI AND NOT CLR_CMAKE_TARGET_ANDROID AND NOT CLR_CMAKE_TARGET_BROWSER)
    if(NOT (CLR_CMAKE_HOST_OS STREQUAL windows))
        if(NOT (CLR_CMAKE_HOST_OS STREQUAL linux AND CLR_CMAKE_TARGET_KOS))
    ...
    if(NOT ((CLR_CMAKE_HOST_ARCH_AMD64 AND (CLR_CMAKE_TARGET_ARCH_AMD64 OR CLR_CMAKE_TARGET_ARCH_ARM64)) OR (CLR_CMAKE_HOST_ARCH_I386 AND CLR_CMAKE_TARGET_ARCH_ARM)))
        if(NOT (CLR_CMAKE_TARGET_KOS AND CLR_CMAKE_HOST_ARCH_ARM64 AND CLR_CMAKE_TARGET_ARCH_ARM64))
