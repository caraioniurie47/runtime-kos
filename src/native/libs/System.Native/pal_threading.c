...
#if defined(__KOS__)
#include <coresrv/sync/sync.h> /* KnFutexWait, KnFutexWake */
#include <rtl/rtc.h>           /* INFINITE_TIMEOUT */
...
{
    KnFutexWait((rtl_uint32_t*)address, (rtl_uint32_t)comparand, INFINITE_TIMEOUT, RTL_NULL);
}
...
int32_t SystemNative_LowLevelFutex_WaitOnAddressTimeout(int32_t* address, int32_t comparand, int32_t timeoutMilliseconds)
{
    ...
    Retcode waitResult = KnFutexWait((rtl_uint32_t*)address, (rtl_uint32_t)comparand, (rtl_uint32_t)timeoutMilliseconds, RTL_NULL);

    // normal/immediate (rcFutexWouldBlock)/spurious wakes are not timeouts
    return waitResult != rcTimeout;
}

void SystemNative_LowLevelFutex_WakeByAddressSingle(int32_t* address)
{
    KnFutexWake((rtl_uint32_t*)address, 1, RTL_NULL);
}
#elif defined(TARGET_LINUX)
void SystemNative_LowLevelFutex_WaitOnAddress(int32_t* address, int32_t comparand)
{
